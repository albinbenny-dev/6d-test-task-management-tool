...
import { resolveScriptPaths } from './scriptPaths.js';
...
export function registerSchedule(schedule: ScheduleRow): void {
  ...
  const task = cron.schedule(schedule.cronExpression, async () => {
    ...
    try {
      ...
      const resolvedPaths = await resolveScriptPaths(schedule.projectId, scriptedTcIds);
      const pathByTcId = new Map(resolvedPaths.map((r) => [r.testCaseId, r.scriptPath]));
      ...
      await addRunJob({
        ...
        scriptPaths: scriptedTcIds.map((id) => pathByTcId.get(id)!),
        skippedTcIds,
        environment: schedule.environment,
