...
import { blockAutomationAccess } from '../middleware/rbac.js';
import {
  generateReport,
  getProjectStats,
  getRunTrend,
...
router.use(blockAutomationAccess as RequestHandler);
...
router.get('/runs', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (req.query['triggerType']) where['triggerType'] = req.query['triggerType'] as string;
    if (req.query['name']) where['name'] = req.query['name'] as string;
...
// ── GET /runs/:runId/tc-report ──────────────────────────────────────────────
// TC Library-shaped view of this run: one row per linked TcItem, joined through
// TcItem.linkedScriptId -> TestCase -> RunResult (scoped to this run only).
// Only TcItems with an automation script linked are included — an item whose
// script wasn't part of this particular run shows as NOT_RUN, not omitted.

router.get('/runs/:runId/tc-report', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const run = await prisma.run.findFirst({
      where: { id: req.params['runId'], projectId: req.project.id },
      select: { id: true },
    ...
    const tcItems = await getRunTcReport(req.project.id, req.params['runId']!);
    const items = tcItems.map((item) => ({
      tcItemId: item.tcItemId,
      tcId: item.tcId,
      module: item.module,
      feature: item.feature,
      title: item.title,
      description: item.description,
      scriptTcId: item.scriptTcId,
      execStatus: item.execStatus,
      resultId: item.resultId,
      duration: item.duration,
      errorMessage: item.errorMessage,
      hasScreenshot: !!item.screenshotPath,
      hasVideo: !!item.videoPath,
      hasRfReport: !!item.rfReportPath,
      hasRfLog: !!item.rfLogPath,
    }));
...
// ── GET /suite-groups ────────────────────────────────────────────────────────
// All suite-triggered run groups (by run.name) with aggregate stats — powers
// the Automation Dashboard's Suite browser (unlike getTopSuites, not capped to 5).

router.get('/suite-groups', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const groups = await getSuiteGroups(req.project.id);
    res.json({ groups });
  } catch (err) { next(err); }
});
...
router.get('/runs/:runId/export', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // SHEET 2: Scripts (per automation script/TestCase — as executed in this run)
    // ════════════════════════════════════════════════════════════════════════
    ...
    // ════════════════════════════════════════════════════════════════════════
    // SHEET 3: Test Cases (TC Library-shaped — one row per linked TC Library item)
    // ════════════════════════════════════════════════════════════════════════
    const tcLibItems = await getRunTcReport(run.projectId, run.id);
    const ws3: Record<string, WsCell | object> = {};

    function w3(addr: string, value: string | number, style: CellStyle) {
      ws3[addr] = cell(value, style);
    }

    const tcLibHeaders = ['TC ID', 'Module', 'Feature', 'Test Case', 'Description', 'Status', 'Duration', 'Error', 'Automation Script Ref'];
    tcLibHeaders.forEach((h, idx) => {
      const addr = `${xlsxCol(idx + 1)}1`;
      w3(addr, h, { ...bgSolid(COL.navyDark), ...fontWhiteBold, ...alignHdrWrap });
    });

    tcLibItems.forEach((item, idx) => {
      const rowNum  = 2 + idx;
      const rowFill = idx % 2 === 0 ? COL.lightBlue : COL.white;
      const rowData: Array<string | number> = [
        item.tcId ?? '',
      ...
      rowData.forEach((val, ci) => {
        const addr = `${xlsxCol(ci + 1)}${rowNum}`;
        const baseStyle: CellStyle = { ...bgSolid(rowFill), ...alignWrap };
        w3(addr, val, ci === 5 ? { ...baseStyle, ...fontBold } : baseStyle);
      });
    });

    ws3['!ref'] = `A1:I${1 + tcLibItems.length}`;
    ws3['!cols'] = [
      { wch: 14 }, { wch: 16 }, { wch: 18 }, { wch: 42 }, { wch: 50 },
      { wch: 11 }, { wch: 10 }, { wch: 45 }, { wch: 20 },
    ];
    ws3['!rows'] = [{ hpt: 26 }, ...tcLibItems.map(() => ({ hpt: 20 }))];

    // ── Build workbook and send ───────────────────────────────────────────────
    const wb = XLSXStyle.utils.book_new();
    XLSXStyle.utils.book_append_sheet(wb, ws1 as XLSXStyle.WorkSheet, 'Dashboard');
    XLSXStyle.utils.book_append_sheet(wb, ws2 as XLSXStyle.WorkSheet, 'Scripts');
    XLSXStyle.utils.book_append_sheet(wb, ws3 as XLSXStyle.WorkSheet, 'Test Cases');
