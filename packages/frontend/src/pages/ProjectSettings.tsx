...
function DetailsTab() {
  ...
  async function handleSave() {
    ...
    try {
      await updateProject.mutateAsync({ name, description: desc });
...
function MembersTab() {
  ...
  const [inviteRole, setInviteRole]     = useState<'ADMIN' | 'SUPER_USER' | 'STANDARD_USER' | 'TEST_USER'>('SUPER_USER');
...
// Credentials (JIRA_HOST/JIRA_EMAIL/JIRA_API_TOKEN) are a server-wide env-var
// setting, not editable here — this tab only manages per-project settings
// (which Jira project's issues are relevant, polling cadence, on/off).
function JiraSettingsTab() {
  const { activeProject } = useProjectStore();
  const projectId = activeProject?.id ?? '';
  ...
  useEffect(() => {
    if (data?.config) {
      setJiraProjectKey(data.config.jiraProjectKey ?? '');
      setPollIntervalMinutes(data.config.pollIntervalMinutes);
  ...
  async function handleSave() {
    try {
      await updateConfig.mutateAsync({ jiraProjectKey: jiraProjectKey.trim() || null, pollIntervalMinutes, isEnabled });
  ...
  async function handleTest() {
    ...
    if (result.ok) toast.success(`Connected as ${result.accountName ?? 'Jira user'}`);
  ...
  if (isLoading) return <div style={{ color: 'var(--text-dim)', fontSize: '12px' }}>Loading…</div>;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <div className="card" style={{ padding: '16px' }}>
        <div className="card-header">
          ...
          <span className={`badge ${data?.credentialsConfigured ? 'badge-pass' : 'badge-fail'}`}>
            {data?.credentialsConfigured ? 'Server credentials configured' : 'Server credentials missing'}
          </span>
        </div>
        {!data?.credentialsConfigured && (
          <p style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '12px' }}>
            An administrator needs to set JIRA_HOST, JIRA_EMAIL, and JIRA_API_TOKEN on the server before this project can sync Jira issues.
          </p>
        )}

        <label style={LABEL_STYLE}>Jira project key (optional filter)</label>
        <input
          className="input-field"
        ...
        <label style={LABEL_STYLE}>Poll interval (minutes)</label>
        <input
          className="input-field"
          type="number"
          min={1}
          ...
          style={{ marginBottom: '12px', width: '120px' }}
        />

        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: 'var(--text)', marginBottom: '16px', cursor: 'pointer' }}>
          <input type="checkbox" checked={isEnabled} onChange={(e) => setIsEnabled(e.target.checked)} />
          Enable automatic polling for this project
        </label>

        {data?.config.lastPollAt && (
          <p style={{ fontSize: '11px', color: 'var(--text-dim)', marginBottom: '12px' }}>
            Last synced {new Date(data.config.lastPollAt).toLocaleString()} — {data.config.lastPollStatus}
          </p>
        )}
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <TbBtn variant="ghost" onClick={() => void handleTest()} disabled={testConnection.isPending}>
            {testConnection.isPending ? 'Testing…' : 'Test Connection'}
          </TbBtn>
          <TbBtn variant="ghost" onClick={() => void handleSyncNow()} disabled={syncNow.isPending || !data?.credentialsConfigured}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <Topbar
          breadcrumbs={[
            ...
            { label: activeProject?.name ?? slug ?? 'Project', href: `/projects/${slug}/test-cycles` },
            { label: '⚙ Settings' },
          ]}
        />
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px', color: 'var(--text-mid)' }}>
          <div style={{ fontSize: '40px' }}>🔒</div>
          <div style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)' }}>Access restricted</div>
          <div>Project Settings is only available to SUPER_USER and ADMIN roles.</div>
        </div>
      </div>
    );
  }
