...
export function useReportRuns(
  ...
  page = 1,
  filters?: { triggerType?: string; name?: string },
) {
  return useQuery({
    queryKey: ['report-runs', projectId, page, filters],
    queryFn: async () => {
...
// ── Suite groups (all suite-triggered run groups, for the Suite browser) ───

export function useSuiteGroups(projectId: string | undefined) {
  return useQuery({
    queryKey: ['suite-groups', projectId],
    queryFn: async () => {
      const res = await api.get<{ groups: TopSuiteEntry[] }>(`/projects/${projectId}/reports/suite-groups`);
...
// ── TC Library-shaped report for a run ─────────────────────────────────────
...
export function useTcReport(projectId: string | undefined, runId: string | null, enabled: boolean) {
  return useQuery({
    queryKey: ['tc-report', projectId, runId],
    queryFn: async () => {
      const res = await api.get<{ items: TcReportItem[] }>(
        `/projects/${projectId}/reports/runs/${runId}/tc-report`,
      );
