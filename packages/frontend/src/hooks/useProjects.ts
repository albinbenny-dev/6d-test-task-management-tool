...
import type { Project, EnvConfig, ProjectMember, RequirementDoc, ProjectMemberSearchResult } from '../types';
...
// Searches WITHIN existing project members (opposite of useUserSearch, which
// excludes them for the invite flow) — used for the test-cycle assignee picker.
export function useProjectMemberSearch(projectId: string, query: string) {
  return useQuery({
    queryKey: ['member-search', projectId, query],
    queryFn: async () => {
      if (!projectId || query.trim().length < 2) return [];
      const res = await api.get<{ members: ProjectMemberSearchResult[] }>(
