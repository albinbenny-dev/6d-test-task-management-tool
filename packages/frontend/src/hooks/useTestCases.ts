...
export function useTestCase(projectId: string | undefined, tcId: string | undefined) {
  return useQuery({
    queryKey: ['test-case', projectId, tcId],
    queryFn: async () => {
      const res = await api.get<{ testCase: TestCase }>(`/projects/${projectId}/test-cases/${tcId}`);
      return res.data.testCase;
    },
    enabled: !!projectId && !!tcId,
  });
}
...
export function useUpdateTestCase(projectId: string) {
  ...
  return useMutation({
    ...
    onSuccess: (_data, vars) => {
      qc.invalidateQueries({ queryKey: ['test-cases', projectId] });
      qc.invalidateQueries({ queryKey: ['test-case', projectId, vars.tcId] });
    },
  });
}
